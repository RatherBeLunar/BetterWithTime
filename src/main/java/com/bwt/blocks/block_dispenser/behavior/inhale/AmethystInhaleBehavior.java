...
public class AmethystInhaleBehavior implements BlockInhaleBehavior {
    ...
    public ItemStack getInhaledItems(BlockSource blockPointer) {
        ...
        if (droppedStacks.size() > 1) {
            return droppedStacks.stream()
                    .filter(dropStack -> !dropStack.getItem().equals(state.getBlock().getCloneItemStack(blockPointer.level(), blockPointer.pos(), state).getItem()))
    ...
    public void inhale(BlockSource blockPointer) {
        ...
        if (!(facingState.is(Blocks.AMETHYST_CLUSTER))) {
            return;
        }
